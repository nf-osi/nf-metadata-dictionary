...
Either way an `entity_findings.manifest.json` beside the file records whether the pass completed, how many entities it holds, which projects it inspected, which it never reached and which it cut short part-way.
`fix_annotation_keys.py --findings` reads that manifest and **refuses an `--apply` run** whose input came from a pass that stopped early, unless `--allow-incomplete-findings` accepts the gap; a dry run proceeds, records the gap in `report.csv` and `progress.jsonl`, and exits 2.
...
6. **Every per-entity loop aborts on a systemic failure**, and they all abort through the same guard: the preflight's planning pass, its conformance pass, the write pass, the rollback and verify passes, and the audit's drill-down.
   ...
   Nor does a request the service answered definitively - a 403 on a file behind a per-folder ACL, a 404 on an entity deleted since the audit - which is recorded as lost coverage and left out of the window.
   ...
   Nothing is hidden by that - a definitive failure is still an error in the report and still spends the coverage budget - and a write run with no access at all is refused up front by the permission check rather than by the guard.
