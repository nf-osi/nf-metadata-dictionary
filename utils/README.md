...
Either way an `entity_findings.manifest.json` beside the file records whether the pass completed, how many entities it holds and which projects it inspected.
`fix_annotation_keys.py --findings` reads that manifest and says loudly when its input came from a pass that stopped early.
