...
schema <- read.csv(params$schema_csv) %>%
  select(Attribute, Description, Required, Valid.Values, DependsOn, Parent, 
         ID = .ID, Type = .Type, Root = .Root, SubOf = .SubOf,
         Range = .Range, EditorNote = .EditorNote)


# docTemplate needs the unmodified schema table
if(!params$use_cache) {
  docTemplate(templates = c(# genomics/transcriptomics
                            ...
                            DynamicLightScatteringTemplate = "Dynamic_Light_Scattering_Template"
                            ),
              schema_csv = schema
              )
}
...
```{r schema_ext_assay_template, out.width="100%", echo=FALSE, eval=TRUE}
...
```{r platforms_table, echo=FALSE, eval=TRUE}
...
#### Data Level {#Data .tabset .tabset-fade .tabset-pills}

##### Terms

```{r data_subtypes_table, echo=FALSE}

data_level_table <- schema %>%
  filter(Parent == "dataSubtype") %>%
  select(Attribute, Description, DependsOn)

basicTable(data_level_table)
```
...
data_formats_table <- schema %>%
  ...
  select(Attribute, Description, Note = SubOf)
...
cell_table <- schema %>%
  filter(SubOf %in% c("Primary_Cell", "Cell_Line_Model")) %>%
  select(Attribute, Description, DependsOn)
...
tumor_table <- schema %>%
  filter(Parent == "tumorType" | Parent == "Tumor") %>%
  select(Attribute, Description, DependsOn)
...
templateTable(read.csv("templates/Dynamic_Light_Scattering_Template.csv"))
