...
| Output | Contents |
|--------|----------|
| `data.json` | Templates, slots (with usage-slot overrides), enums, DCA public-template flags |
...
- `dca-template-config.json` (public template list)
...
The GitHub Actions workflow (`.github/workflows/publish-docs.yml`) runs on pushes/PRs to `main` that touch `modules/**`, `docs/**`, or `dca-template-config.json`. It sets up Node 20 and runs `npm ci && node build.mjs`, then deploys to GitHub Pages (prod) or a PR preview.
